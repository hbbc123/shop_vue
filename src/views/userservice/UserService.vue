<template>
    ...
    <div class="bgbox">
        <div class="center">
            <div class="center_left">
                <div class="center_left_redbox">
                    <div style="display: flex;">
                        <img :src="http + data.info.headimg" alt="">
                        <div v-if="data.info" class="center_left_redbox_name">
                            {{ data.info.nickname }}
                        </div>
                    </div>
                ...
                <div class="center_left_list">
                    <div @click="FnShowList(key)" v-for="(item, key) in data.list_shop" class="center_left_list_box"
                        :class="data.key == key ? 'active' : ''">
                        <a-badge :dot="item.read_tf == 1">
                            <div :style="{ backgroundImage: `url(${http + item.shop_img})` }" class="avatar ss">
                            </div>
                        </a-badge>
            ...
            <div class="center_rgth" v-show="data.list.length > 0">
                <div class="center_rgth_top">
                    <div class="center_rgth_top_box">
                        <div class="center_rgth_top_img"> </div>
                        <div v-if="data.list.length > 0" class="center_rgth_top_name">
                            {{ data.list_shop[data.key].shop_name }}
                        </div>
                    </div>
                </div>
                    ...
                    <div v-if="data.list[0]">
                        <div v-for="(item, key) in data.list" :key="item.send_time">
                            <div class="ji" v-if="item.both == 0">
                                <p class="message--pin  tal">{{ item.service_nickname }}</p>
                            ...
                            <div v-else>
                                <p class="message--pin tar ">{{ item.user_nickname }}</p>
                                <div class=" message--content clearfix">
                                    <div class="message--wrapper">
                                        <div class="text-message text-messagess clearfix waiter-message oo" index="9">
                                            <div class="text-message--container">
                                                <p>{{ item.service_info }}</p>
                                                <div v-if="item.fujian">
                                                    <img v-if="item.fujian.type == 'img'" class="in_img"
...
const data = reactive({ list_shop: [], list: [], key: -1, info: {}, str: '' })
...
if (-1 != parseInt(id)) {
    console.log(777);
    request.get('/index/index/inquireshop', { shop_id: id }).then(res => {
        console.log(res, 999);
        if (res.data.code == 200) {
            serve.send(JSON.stringify({ constructor: 3, data: { user_id: data.info.user_id, } }));
        }
    })
}
function FnEnter() {
...
function FnShowList(k) {
    data.key = k
    data.list_shop[k].read_tf = false
    console.log(k)
    serve.send(JSON.stringify({ constructor: 2, data: { shop_id: data.list_shop[k].shop_id, user_id: data.info.user_id, type: 1 } }));
}
function FnShowFile() {
    console.log(555);
...
function FnSetMess(file = false) {
    ...
    formData.append('shop_id', data.list_shop[data.key].shop_id)
    formData.append('info', data.str)
    formData.append('user_id', data.info.user_id)
    formData.append('both', 1)
    if (file) {
    ...
    request.post_file('/index/index/setserveinfo', formData).then(res => {
        if (res.data.code == 200) {
            ...
            data.str = ''
            serve.send(JSON.stringify({ constructor: 2, data: { user_id: data.info.user_id, shop_id: data.list_shop[data.key].shop_id, type: 1 } }));
            if (res.data.data) {
                serve.send(JSON.stringify({ constructor: 5, data: { shop_id: data.list_shop[data.key].shop_id } }));
...
function FnMessage(e) {
    ...
    switch (info.type) {
        case 1: serve.send(JSON.stringify({ constructor: 3, data: { user_id: data.info.user_id, } })); break;
        case 3: FnTypeThree(info.data); break;//获取聊天记录
        case 4: data.list_shop = info.data;
            console.log(data.key);
            if (data.key != -1) {
                data.list_shop[data.key].read_tf = false
                serve.send(JSON.stringify({ constructor: 2, data: { shop_id: data.list_shop[data.key].shop_id, user_id: data.info.user_id, type: 1 } }));
            }
            if (parseInt(id) != -1) {
                data.list_shop.forEach((val, k) => {
                    if (val.shop_id == parseInt(id)) {
                        data.key = k
                        serve.send(JSON.stringify({ constructor: 2, data: { shop_id: data.list_shop[data.key].shop_id, user_id: data.info.user_id, type: 1 } }));
                    }
                })
